...
const csv = `Position,Symbol,Direction,Open Time,Close Time,Entry Price,Exit Price,Quantity,PnL,Fees
1,EURUSD,long,2026-01-05 09:00,2026-01-05 10:00,1.1,1.102,0.1,18.5,1.5
2,EURUSD,long,2026-01-05 09:00,2026-01-05 10:00,1.1,1.102,0.1,18.5,1.5`;
beforeEach(() => {
...
2026-01-05 10:00,AAPL,sell,2,101`;
    ...
    expect(insertExecutions("test", parsed.executions, "import")).toEqual({
      inserted: 3,
      duplicates: 0,
    });
    expect(insertExecutions("test", parsed.executions, "import")).toEqual({
...
1,Exit long,2026-01-05 10:00,101,1,1`;
  ...
  it("does not replace reported zero fees or P&L with account defaults or multiplier recalculations", () => {
    ...
    db.insert(settings)
      ...
      .run();
    const parsed =
      parseAuto(`Position,Symbol,Direction,Open Time,Close Time,Entry Price,Exit Price,Quantity,PnL,Fees
