...
import type { JournalDefaults } from "@/lib/journal-defaults";
...
export const POST = handler(async (request: Request, { params }: Context) => {
  ...
  if (resource === "defaults") {
    requireValue(
      finite(b.breakeven) && b.breakeven >= 0 && ["money", "percent"].includes(b.breakevenMode),
    ...
    for (const key of ["feeRules", "riskRules"] as const) {
      requireValue(
        Array.isArray(b[key]) && b[key].length <= 100,
        "Use at most 100 defaults per type.",
      );
      for (const r of b[key]) {
        requireValue(
          text(r.id, 100) &&
            text(r.accountId, 100) &&
            (!r.accountId || knownAccounts.has(r.accountId)) &&
            text(r.symbol, 80),
          "Invalid default account or symbol.",
        );
        if (key === "feeRules")
          requireValue(
            finite(r.amount) && r.amount >= 0 && ["unit", "execution"].includes(r.mode),
          ...
          requireValue(
            finite(r.stop) &&
              ...
              finite(r.target) &&
              r.target > 0 &&
              ["price", "percent"].includes(r.mode),
            "Stop and target distances must be positive.",
          );
      }
    }
    setSetting("journalDefaults", JSON.stringify(b as JournalDefaults));
    return ok({ saved: true });
  }
  return bad("Unknown resource", 404);
});
